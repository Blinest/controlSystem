...
uint16_t cmd_packer_pack_status_frame(uint8_t* frame, GlobalMotor motor[MOTOR_NUM] , uint8_t state) {
    ...
    for (int i = 0; i < MOTOR_NUM; i++) {
        int16_t pos = (int16_t)(motor[i].motor.dc_motor.current_pos * 100);
        int16_t vel = (int16_t)(motor[i].motor.dc_motor.current_speed * 100);
    ...
    //截面控制
    int16_t s_val = (int16_t)(LYZ.current_S * 100);
    frame[idx++] = (s_val >> 8) & 0xFF; frame[idx++] = s_val & 0xFF;

	//偏转控制
	int16_t s_val2 = (int16_t)(LYZ.current_phi * 100);
...
{
    if (frame == NULL || frame_len == 0U || SensorMessageQueueHandle == NULL) return;

    for (uint16_t i = 0; i < frame_len; i++) {
        (void)osMessageQueuePut(SensorMessageQueueHandle, &frame[i], 0, 0);
    }
}
...
void cmd_packer_send_status_frame(void)
{
    ...
    frame_len = cmd_packer_pack_status_frame(frame, global_motor, 1);
    Usart_SendString(&huart2, frame, frame_len);
}