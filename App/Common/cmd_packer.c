									  ...
									  uint8_t state)
{
    ...
    for (int i = 0; i < MOTOR_NUM; i++) {
    	...
    	const StepperMotor* sm = &motor->stepper_motor;
        const int16_t pos = (int16_t)(sm->current_pos * 100); // mm
        const int16_t vel = (int16_t)(sm->current_vel * 100); // mm/s
        const int16_t acc = (int16_t)(sm->current_acc * 100); // mm/s^2
    	const int8_t motor_state = (int8_t)(motor->state);
    ...
    for (int i = 0; i < SENSOR_NUM; i++) {
        int16_t x = (int16_t)(sensor[i].global_sensor.press_sensor.val * 100);
        int16_t y = (int16_t)(sensor[i].global_sensor.imu.roll * 100);
        int16_t z = (int16_t)(sensor[i].global_sensor.imu.yaw * 100);
