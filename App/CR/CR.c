...
void deltaL_update(void)
{
    ...
    for(int i = 1; i <= 6; i++)
    {
        cur_pos[i] = CR.joint_space.target_deltaL[i];
    }
    calculate_L(CR.parameter.r, CR.joint_space.target_theta, CR.joint_space.target_phi, CR.joint_space.target_deltaL);
    motor_sync_control(MOTOR_NUM,0, CR.joint_space.target_deltaL);
    // char response[128];
...
void autoStraight(void)
{
    ...
    CR.joint_space.current_theta[1] = 0;
    CR.joint_space.target_phi[0] = 0;
    CR.joint_space.current_phi[1] = 0;
    deltaL_update();
}
...
uint8_t armBend_edit(int seg, char direction, double val, double g_u, double g_r, double g_d, double g_l, double seg1_limit, double seg2_limit)
{
    ...
    // 更新补偿后的关节角度
    CR.joint_space.target_theta[seg-1] = compensated_angle_rad;
    CR.joint_space.target_phi[seg-1] = phi;
    deltaL_update();
    return 0;
}
