...
// ✅ 修复：将大的栈分配改为全局静态缓冲区，防止堆栈溢出
static uint8_t g_offbuf[32*6];  // 全局 192 字节缓冲，而不是栈上分配
void WritePos(uint8_t ID, uint16_t Position, uint16_t Time, uint16_t Speed)
{
	uint8_t bBuf[6];
	...
	genWrite(ID, SCSCL_GOAL_POSITION_L, bBuf, 6);
}
...
{
  // ✅ 修复：使用全局缓冲区而不是栈上分配，防止堆栈溢出
  uint8_t i;
  
  // 清空全局缓冲区
  memset(g_offbuf, 0, sizeof(g_offbuf));
  ...
  for(i = 0; i<IDN; i++){
    ...
    Host2SCS(g_offbuf+i*6+0, g_offbuf+i*6+1, Position[i]);
    Host2SCS(g_offbuf+i*6+2, g_offbuf+i*6+3, T);
    Host2SCS(g_offbuf+i*6+4, g_offbuf+i*6+5, V);
  }
  syncWrite(ID, IDN, SCSCL_GOAL_POSITION_L, g_offbuf, 6);
}
...
int PWMMode(uint8_t ID)
{
	...
	bBuf[2] = 0;
	bBuf[3] = 0;
	genWrite(ID, SCSCL_MIN_ANGLE_LIMIT_L, bBuf, 4);
}
