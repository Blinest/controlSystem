...
void ZDT_X42_V2_Receive_Data(uint8_t *rxCmd, uint8_t *rxCount)
{
	...
	if (HAL_CAN_GetRxMessage(&hcan, CAN_RX_FIFO0, &RxHeader, rx_data) == HAL_OK)
	{
		*rxCount = RxHeader.DLC;
		// 注意：原代码中 rxCmd[0] 错误地使用了 can.CAN_TxMsg.ExtId（发送ID），这里修正
		// 如果你需要获取接收到的 CAN ID，可以从 RxHeader 获取
		rxCmd[0] = (uint8_t)(RxHeader.StdId >> 8);   // 仅示例，具体根据你的协议定义
